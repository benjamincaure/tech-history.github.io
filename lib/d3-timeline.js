...
var timelines = function () {
	...
	function timelines(gParent) {
		...
		var xAxis;
		if (orient == "bottom") {
			xAxis = d3.axis.axisBottom();
		} else if (orient == "top") {
			xAxis = d3.axis.axisTop();
		}
		if (timeIsLinear) {
			xScale = d3.scale.scaleLinear()
				.domain([beginning, ending])
				.range([margin.left, width - margin.right]);

			xAxis.scale(xScale)
				.tickFormat(formatDays)
				.tickValues(d3.array.range(0, ending, 86400));
		} else {
			xScale = d3.scale.scaleTime()
				.domain([beginning, ending])
				.range([margin.left, width - margin.right]);
